import { Cloud, BarChart3, Folder, Rocket, Server, Settings, HelpCircle, User } from "lucide-react";
    ...
    <div className="w-64 bg-slate-900 border-r border-slate-700 flex flex-col">
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        {settingsItems.map((item) => (
          <button
            key={item.id}
            className="nav-link w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left"
          >
            <item.icon className="w-5 h-5" />
            <span>{item.label}</span>
          </button>
